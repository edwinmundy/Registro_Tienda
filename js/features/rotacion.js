...
const App = (function() {
    ...
    function obtenerCategoriasConProductosRotacion() {
        ...
        return obtenerCategorias()
            .map(categoria => ({
                ...categoria,
                productos: obtenerDatos(categoria.id)
            }))
    ...
    function guardarLeyendasStorage() {
        localStorage.setItem(CONFIG.leyendaKey, JSON.stringify(state.leyendas));
    }
    ...
    function guardarLocalStorage() {
        try {
            localStorage.setItem(CONFIG.storageKey, JSON.stringify(state.bandejas));
            localStorage.setItem(CONFIG.historialKey, JSON.stringify(state.historial));
        } catch (e) {
    ...
    function crearCirculo(producto, bandeja) {
        ...
        if (producto.estado === 'libre') {
            ...
            if (producto.estado === 'disponible') {
                if (horas >= CONFIG.alertaHoras) {
                    timerClass = 'timer-danger';
                    timerIcon = '!';
                    div.classList.add('alerta-tiempo');
                } else if (horas >= CONFIG.alertaHoras * 0.75) {
    ...
    function mostrarTooltip(e, producto, bandeja) {
        ...
        document.getElementById('ttTiempo').textContent = calcularTiempoTranscurrido(producto.fechaExhibicion);
                ...
                if (p.estado === 'disponible' && p.fechaExhibicion) {
                    ...
                    if (horas >= CONFIG.alertaHoras) {
                        alertas.push({
                            ...
                            tiempo: `${Math.floor(horas)}h ${Math.floor((horas % 1) * 60)}m exhibido`,
                            horas: horas
                        });
                    } else if (horas >= CONFIG.alertaHoras * 0.75) {
                        alertas.push({
